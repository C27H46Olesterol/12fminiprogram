...
Page({
  ...
  onLoad(options) {
    console.log('🚀 manager/rate-issue onLoad - 接收到的参数:', options);
    const { id, readonly } = options;
    ...
    if (id) {
      ...
      console.log('✅ 设置 readonly:', readonly === 'true');
      this.initPage();
      this.loadIssueDetail();
    } else {
      console.error('❌ 缺少工单ID');
      wx.showToast({
        title: '缺少工单ID',
        icon: 'error'
      });
  ...
  async loadIssueDetail() {
    try {
      ...
      if (result.result && result.result.success) {
        ...
        // 如果已经评价过，显示之前的评价
        if (data.issue.managerSatisfaction) {
          this.setData({
            rating: data.issue.managerSatisfaction,
            feedback: data.issue.managerFeedback || ''
          });
        }
        
        console.log('📋 工单详情加载完成');
